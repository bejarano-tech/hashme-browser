...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  gclientSync: (forceReset = false, cleanup = false, shouldIgnoreChromium = false, braveCoreRef = null, options = {}) => {
    ...
    if (util.shouldUpdateChromium()) {
      // Separate shouldUpdate and shouldIgnore so
      // that we still log the need to update.
      if (!shouldIgnoreChromium) {
        args = [...args, ...chromiumArgs]
        reset = true
      } else {
        console.log(chalk.yellow.bold(`Not updating chromium since instructed to ignore via a flag.`))
      }
    }
